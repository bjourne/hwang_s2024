...
parser.add_argument('--model', '-m', metavar='NAME', default='relu_swin_tiny_patch4_window7_224',
...
def validate(args):
    ...
    top1 = AverageMeter()
    top5 = AverageMeter()
    model,n_layer = replace_identity_by_module(model,0,int(args.batch_size/args.num_gpu))
    model.eval()
    with torch.no_grad():
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            # import numpy as np
            # np.savetxt('ann.txt', input[0][0].cpu().flatten().numpy())
            # assert False, input
    ...
    return results,model,loader



def snn_validate(args,model,loader):
    # might as well try to validate something
    args.pretrained = args.pretrained or not args.checkpoint
    args.prefetcher = not args.no_prefetcher
    ...
    model.eval()
    print("Timestep:  ",args.timestep, "   base:  ",args.base, "# of layer:",n_layer)
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            for t in range((n_layer-1)*wait+timestep):
                ...
                if(t<timestep ):
                    images_tp *=2.0
                    # tp*=2
                    input_spike = torch.where(images_tp.abs()>=1,1,0)*input_sign
                    images_tp = torch.where(input_spike!=0, images_tp-input_spike, images_tp)
                    # tp+= input_spike*input.max()
                # elif(t<timestep+wait):
                #     input = torch.zeros_like(images_tp)
                #     output = model(input*images.max())
                    # if(t==timestep-1):
                    #     import numpy as np
                    #     #np.savetxt('snn.txt', (self.tp*self.scale).cpu().flatten().numpy())
                    #     np.savetxt('snn.txt', (tp[0][0]*input.max()).cpu().flatten().numpy())
                    #     assert False, input.max()
            ...
            # import numpy as np
            # np.savetxt('snn.txt', (output.cpu().flatten().numpy()))
            # assert False, "stop"
            ...
            if batch_idx % args.log_freq == 0:
                ...
                _logger.info(
                    'Test: [{0:>4d}/{1}]  '
                    'Time: {batch_time.val:.3f}s ({batch_time.avg:.3f}s,/s)  '
                    'Acc@1: {top1.val:>7.3f} ({top1.avg:>7.3f})  '
                    'Acc@5: {top5.val:>7.3f} ({top5.avg:>7.3f})'.format(
                        batch_idx,
                        len(loader),
...
def main():
    ...
    if len(model_cfgs):
        ...
        if args.retry:
            ...
            results,model,loader = validate(args)
    ...
    if len(model_cfgs):
        ...
        if args.retry:
            ...
            results = snn_validate(args,model,loader)
