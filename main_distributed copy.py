...
group.add_argument('--model', default='relu_swin_tiny_patch4_window7_224', type=str, metavar='MODEL',
...
def main():
    utils.setup_default_logging(log_path= './result_sample/output.log')
...
def validate(
    ...
    logger.info(f"Energy (mJ): {flops / 1e9 * 4.6}")
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                ...
                for t in range((n_layer-1)*wait+timestep):
                    if(t<timestep ):
                        ...
                        input_count+= torch.count_nonzero(input_spike)/(input_spike.flatten()).size()[0]
            ...
            if utils.is_primary(args) and (last_batch or batch_idx % args.log_interval == 0):
                ...
                logger.info(f"Energy of SNN (mJ): {energy / 1e9 *0.9}")
