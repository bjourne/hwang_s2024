...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        if self.fused_attn:
            ...
            # np.savetxt('ann.txt', (attn)[0][0].cpu().flatten().numpy())
            # assert False, (attn)[0][0][0]
            # attn_tp = (attn.abs()/attn.abs().max(-1).values.unsqueeze(-1))
            ...
            if(attn is not None):
                if(self.snn_mode):
                    ...
                    attn = self.attn_drop(attn)
                    # attn = attn/attn.max(-1).values.unsqueeze(-1)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # import numpy as np
        # np.savetxt('ann.txt', (x_norm)[0].cpu().flatten().numpy())
        # assert False, (x_norm)[0][0]
