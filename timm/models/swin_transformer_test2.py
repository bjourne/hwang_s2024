...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        self.log_base = torch.log(torch.tensor(base))
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        if self.fused_attn:
            ...
            q_check = torch.where(q.max(-1).values.unsqueeze(-1)==q.abs().max(-1).values.unsqueeze(-1) ,1,-1 )
            q = torch.where(q*q_check>0,q,0 )
            q_ab = q.abs()
            q_sign = q.sign()
            #scale_q =  q_ab.max()
            scale_q =  q_ab.max(-1).values.unsqueeze(-1)
 
            q_ab = q_ab/scale_q

            # q = (q*timestep).round().to(torch.cuda.IntTensor).to(torch.cuda.FloatTensor)/timestep*scale_t
            # q = q * self.scale
            tp_log_query = torch.log(q_ab)/self.log_base
            
            k_check = torch.where(k.max(-1).values.unsqueeze(-1)==k.abs().max(-1).values.unsqueeze(-1) ,1,-1 )
            k = torch.where(k*k_check>0,k,0 )

            k_ab = k.abs()
            k_sign = k.sign()
            scale_k =  k_ab.max(-1).values.unsqueeze(-1)

            k_ab = k_ab/scale_k
            tp_log_key = torch.log(k_ab)/self.log_base
            
            # tp_round_q = torch.round(tp_log_query)
            # tp_round_k = torch.round(tp_log_key)
            # _assert(False, f"sum is ({k.shape}) strange({q.shape})")

            tp_round_q = torch.where(tp_log_query<=-1,torch.round(tp_log_query),-1)
            tp_round_k = torch.where(tp_log_key<=-1,torch.round(tp_log_key),-1)


            q_m = torch.where(tp_round_q>=-timestep,base**tp_round_q,0)*q_sign* self.scale*scale_q
            k_m = torch.where(tp_round_k>=-timestep,base**tp_round_k,0)*k_sign*scale_k

            # q_m = base**tp_round_q *q_sign* self.scale*scale_k
            # k_m = base**tp_round_k *k_sign*scale_q
            # k = (k*timestep).round().to(torch.cuda.IntTensor).to(torch.cuda.FloatTensor)/timestep*scale_t
            ...
            # atten_scale = attn.max()
            # attn = attn/atten_scale
            # attn = base**(-torch.round((attn.max()-attn)/(self.log_base/atten_scale))) #*(1/torch.log(torch.tensor(base)))
            # attn = attn/attn.sum()
            # attn2 = self.softmax(attn)

            atten_scale = attn.max(-1).values.unsqueeze(-1) 
            attn = attn/atten_scale
            # _assert(False, f"sum is ({attn.sum(dim=-1).size()}) strange({(attn.max(-1).values.size())})")


            # attn[attn<0]=0
            attn_round = (-torch.round(( 1-attn)/(self.log_base/atten_scale ))) #*(1/torch.log(torch.tensor(base)))
            attn = torch.where(attn_round>=-timestep,base**attn_round,0)


            aa = torch.round(torch.log(attn.sum(dim=-1)/(attn.size()[-1]))/self.log_base) #torch.log(torch.tensor(2)))
            # _assert(False, f"sum is ({aa}) strange({(attn.size()[-1])})")

            norm_aa = torch.where(aa>=-timestep,base**aa*(attn.size()[-1]),1)
            # _assert(False, f"sum is ({norm_aa}) strange({(norm_aa.size())})")

            attn = attn/norm_aa.unsqueeze(-1) 

            # attn = attn/attn.sum(-1).unsqueeze(-1) 

            attn = self.attn_drop(attn)
                        
            # v_check = torch.where(v.max(-1).values.unsqueeze(-1)==v.abs().max(-1).values.unsqueeze(-1) ,1,-1 )
            # v = torch.where(v*v_check>0,v,0 )
            ...
            v_ab = v_ab/scale_v
            tp_log_value = torch.log(v_ab)/self.log_base
            tp_round_v = torch.where(tp_log_value<=-1,torch.round(tp_log_value),-1)
            # tp_round_v = torch.round(tp_log_value)
            v_m = torch.where(tp_round_v>=-timestep,base**tp_round_v,0)*v_sign*scale_v

            # v_m = base**tp_round_v *v_sign

            # v = (v*timestep).round().to(torch.cuda.IntTensor).to(torch.cuda.FloatTensor)/timestep*scale_t
            x = (attn @ v_m)
            # x = attn @ v
        ...
        x_ab = x.abs()
        x_sign = x.sign()
        scale_x =  x_ab.max(-1).values.unsqueeze(-1)

        x_ab = x_ab/scale_x
        tp_log_x = torch.log(x_ab)/self.log_base
        tp_round_x = torch.where(tp_log_x<=-1,torch.round(tp_log_x),-1)
            # tp_round_v = torch.round(tp_log_value)
        x = torch.where(tp_round_x>=-timestep,base**tp_round_x,0)*x_sign*scale_x
        x = self.proj_drop(x)
        return x
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(B, -1, C)
        x = x + self.drop_path2( self.mlp_norm(self.norm2(x)))
        x = x.reshape(B, H, W, C)
        return x
